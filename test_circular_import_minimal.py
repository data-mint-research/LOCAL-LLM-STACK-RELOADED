"""
Minimal test script to verify that the circular import issue has been resolved.
This script creates simplified versions of the modules to test the import structure.
"""
...
test_dir = os.path.join(os.path.dirname(__file__), 'test_modules')
os.makedirs(test_dir, exist_ok=True)

# Create simplified versions of our modules
error_py = os.path.join(test_dir, 'error.py')
logging_py = os.path.join(test_dir, 'logging.py')
log_exceptions_py = os.path.join(test_dir, 'log_exceptions.py')

# Write simplified error.py
with open(error_py, 'w') as f:
    f.write("""
...
# Write simplified log_exceptions.py
with open(log_exceptions_py, 'w') as f:
    f.write("""
...
# Write simplified logging.py
with open(logging_py, 'w') as f:
    f.write("""
from enum import Enum
...
try:
    from test_modules.error import ErrorCode, LLMStackError
    print("Successfully imported error module")
    
    from test_modules.log_exceptions import LogLevelError, LogFileError
    print("Successfully imported log_exceptions module")
    
    from test_modules.logging import LogLevel, set_log_level
